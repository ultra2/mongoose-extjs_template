...
Ext.define('NAT.autopanel.Grid', {
    ...
    btnNew_click: function(){
        ...
        var newModel = app.CreateModel(this.model);
        debugger;
...
Ext.define('natjs.classes.NATEnum', {
}, function() {

//    Ext.Enum = function Enum() {
//        for (var i in arguments) {
//            this[arguments[i]] = i;
//        }
//    };
//    Ext.ModelState = new Ext.Enum('UNDEFINED', 'LOADINGLOADING', 'CONSISTENT');
...
Ext.define('natjs.plugin.TreeEditing', {
...
//    mixins: {
//        editor: 'natjs.view.aceeditor.Editor'
//    },
//EDITOR BEGIN
    path: '',
    sourceCode: '',
...
Ext.define('NAT.form.FieldSet', {
    extend: 'Ext.form.FieldSet',
    alias: 'widget.natfieldset'
});
...
//        },
    ...
    InitBinding: function () {
        ...
        this.mon(this.store, 'modelChanged', function(model, fieldName, value) {
            if (model == this.store.currModel) { //not embedded model
                this.ReadValue(fieldName, value);
            }
        }, this);
...
Ext.define('natjs.overrides.app.Application', {
    ...
    Ext.override(Ext.app.Application, {
        ...
        constructor: function(config) {
            ...
            this.scriptRunner = Ext.create('natjs.NATScriptRunner');
            this.modelNamespace = config.name + '.model.';
            this.cacheModelDeclaredFields = Ext.create('Ext.util.MixedCollection');
        ...
        CreateModel: function(model, config) {
            config = config || {};
            var className = '';
            ...
            var result = Ext.create(fullClassName);
            result.set('_type', className);
            result.set(config);
            result.Created();
            return result;
        }
...
Ext.define('natjs.overrides.data.AbstractStore', {
    ...
    Ext.override(Ext.data.AbstractStore, {
        ...
        sync: function(options) {
            ...
            me.needsSync = false; //IZS

            if (needsSync && me.fireEvent('beforesync', operations) !== false) {
                options = options || {};

                me.needsSync = true; //IZS
        ...
        Load: function (params, callback, scope) {
            ...
            var options = {
                ...
                callback: function (models, operation, success) {
                    this.Loading = false;
                    this.ProcessResponse(models, operation, success);
                    this.fireEvent('LoadCompleted', models, operation, success);
                    Ext.callback(callback, scope, [null, null], 0);
                }
            };
        ...
        Save: function (op, callback, scope) {
            ...
            async.waterfall([
                ...
                function(result, cb){
                    var options = {};
                    options.scope = me;
                    options.success = function(batch, options) {
                        cb(null, options);
                    };
                    options.failure = function(batch, options) {
                        cb('failure', options);
                    };
                    me.sync(options);
                    options.needsSync = me.needsSync;
                    if (!options.needsSync){
                        cb(null, options);
                        return;
                    }
                    if ((options.needsSync) && (!options.preventLoadingMask)) {
                        app.ShowLoadingMask();
                    }
                }
                ...
                me.fireEvent('SaveCompleted', options);
        ...
        OnModelChanged: function (model, fieldName, value){
            this.fireEvent('modelChanged', model, fieldName, value);
        },

//        OnModelChanged: function (model){
//            return function(changedModel, fieldName, value) {
//                this.fireEvent('modelChanged', model, fieldName, value);
//            }
//        },
...
Ext.define('natjs.overrides.data.BelongsToAssociation', {
    ...
    Ext.override(Ext.data.BelongsToAssociation, {
        ...
        constructor: function(config) {
            ...
            Ext.data.BelongsToAssociation.superclass.constructor.call(this, config);

            var ownerProto = me.ownerModel.prototype,
                getterName = me.getterName || 'get' + me.name,
                setterName = me.setterName || 'set' + me.name;

            Ext.applyIf(me, {
                storeName: 'belongsTo_' + me.name,
                foreignKey : me.name + "_id",
                associationKey: me.name
            });
            ownerProto[getterName] = me.createGetter();
            ownerProto[setterName] = me.createSetter();

            ownerProto[me.name] = me.createStore();
        },
        ...
        createStore: function() {
            ...
            return function() {
                ...
                if (me[storeName] === undefined) {
                    ...
                    me[storeName] = Ext.create('widget.natmodelstore', config); //NAT Support!!!
                    if (autoLoad) {
                        me[storeName].load();
                    }

                    //Owner record fires change event when its hasMany association changes
                    me[storeName].on('modelChanged', me.OnChanged, me);
                    me[storeName].on('add', me.OnChanged, me);
                    me[storeName].on('remove', me.OnChanged, me);
                }
        ...
        write: function(record, data){
            ...
            data[this.name] = writer.getRecordData(store.model);
        },
...
Ext.define('natjs.overrides.data.TreeStore', {
...
//Ext.define('natjs.overrides.TreeStore', {
//    requires: 'Ext.data.TreeStore'
//}, function() {
//    Ext.override(Ext.data.TreeStore, {
//
...
Ext.define('natjs.overrides.data.Store', {
    requires: ['Ext.data.Store']
}, function() {
    Ext.override(Ext.data.Store, {

        GetModelByPropertyValue: function(propName, propValue, onlyFirst) {
            var result = [];
            for (var i = 0; i < this.getCount(); i++) {
                var m = this.getAt(i);
                if (m.get(propName) == propValue) {
                    if (onlyFirst) return m;
                    result.push(m);
                }
            }
            return result;
        ...
        GetModelIdByPropertyValue: function(propName, propValue) {
            var m = this.GetModelByPropertyValue(propName, propValue, true);
            if (m) return m.getId();
            return null;
        }
    });
...
Ext.define('natjs.overrides.form.field.Field', {
...
//    getErrors: function(value) {
//        var errors = [];
//        this.fireEvent('getErrors', this, value, errors);
//        return errors;
//    }
    });
});
...
//            this.viewNode.SetConfig('height', height);
        ...
        ParentClasses: function () {
            if (this.self.parentClasses) return this.self.parentClasses;
            var classes = [];
            this.GetParentClasses(classes);
            this.self.parentClasses = classes;
            return this.self.parentClasses;
        },

        GetParentClasses: function (classes) {
            if (typeof this.superclass.GetParentClasses != 'undefined') {
                this.superclass.GetParentClasses(classes);
            }
            classes.push(this);
        },

        IsDescendantOf: function(cls) {
            var r = Ext.Array.contains(this.ParentClasses, cls);
            return r; //Todo: debug this!
        },
...
Ext.define('NAT.tree.Panel', {
    ...
    requires: ['natjs.plugin.TreeEditing'],
    ...
    constructor: function (config) {
        if (this.natEditable) {
            config.plugins = [Ext.create('natjs.plugin.TreeEditing', { clicksToEdit: 0 })];
        }
        this.callParent([config]);
        return this;
    },
...
Ext.define('NAT.data.Model', {
    ...
    constructor: function(data, id, raw) {
        this.callParent(arguments);
        this.state = Ext.ModelState.UNDEFINED;
        this.errors = errors = Ext.create('Ext.data.Errors');
//        this.set('type', this.$className); //it would run on loading!
    ...
    Created: function() {
        for (var i = 0; i < this.associations.length; i++) {
            ...
            association.Created(this);
        }
        this.state = Ext.ModelState.CONSISTENT;
    },
    ...
    IsHierarchy: function() {
        return this.fields.containsKey('parentId');
    },

//    BuildSendData: function (sendData) {
//        sendData.type = this.get('type');
//    },

    getAllValues: function(){
        return this[this.persistenceProperty];
    },

    get: function(fieldName) {
        if ((Ext.isObject(fieldName)) || (this.state != Ext.ModelState.CONSISTENT)) {
...
//            value = this.fields.getByKey(fieldName).defaultValue;
...
//        }
    ...
    set: function (fieldName, value, op) {
        this.callParent(arguments);

        if ((!Ext.isObject(fieldName)) && (this.state == Ext.ModelState.CONSISTENT)) {
            //console.log('SET (' + this.$className + ')' + this.get('name') + "fieldName: " + fieldName + ", value: " + value);
            this.isNATValid = false; //call async ValidateModel to set true

            this.OnChanged(this, fieldName, value, op);
            ...
            if (typeof this[fnSet] == "function") {
                if (Ext.Array.contains(this.skipFieldSetters, fieldName)) {
                    ...
                    Ext.callback(this[fnSet], this, [value], 1);
                }
            }
        }
    },
...
//    bindingIsChanging: false,
//    BindModel: function (srcField, destModel, destField) {
//        this.on('changed', this.GetBindModelWriteFunction(srcField, destModel, destField), this);
//        destModel.on('changed', this.GetBindModelReadFunction(srcField, destModel, destField), this);
//    },
//
//    GetBindModelWriteFunction: function(srcField, destModel, destField) {
//        return function(fieldName, value) {
//            if (fieldName != srcField) return;
//            if (this.bindingIsChanging) return;
//
//            this.bindingIsChanging = true;
//            destModel.set(destField, value);
//            this.bindingIsChanging = false;
//        }
//    },
//
//    GetBindModelReadFunction: function(srcField, destModel, destField) {
//        return function(fieldName, value) {
//            if (fieldName != destField) return;
//            if (this.bindingIsChanging) return;
//
//            this.bindingIsChanging = true;
//            this.set(srcField, value);
//            this.bindingIsChanging = false;
//        }
//    },

    OnChanged: function (model, fieldName, value, op){
        this.UpdateDirty();
        this.fireEvent('changed', model, fieldName, value, op);
    },

    OnDirtyChanged: function (model){
        this.fireEvent('dirtyChanged', model);
    },
        ...
        for (field in modified) {
            if (modified.hasOwnProperty(field)) {
                if (typeof modified[field] != "function") {
                    ...
                    //IZS BEGIN
                    this.OnChanged(field, modified[field]);
                    //IZS END
                }
            }
        }
    ...
    DeclaredFields: function (includeDebugFields) {
        var result = app.cacheModelDeclaredFields.getByKey(this.self.$className);
        if (!result) {
            result = this.GetDeclaredFields();
            app.cacheModelDeclaredFields.add(this.self.$className, result);
        }
        if (!includeDebugFields){
            result = Ext.Array.filter(result, function(item) {
                return !item.debug
            });
        }
        return result;
    ...
    GetDeclaredFields: function () {
        if (this.superclass.fields) {
            if (this.superclass.fields.length == this.fields.length) {
                return [];
            }
            else {
                return this.fields.getRange(this.superclass.fields.length);
            }
        }
        else {
            return this.fields.getRange(0);
        }
    },
...
Ext.define('NAT.data.ModelStore', {
    extend: 'Ext.util.Observable',
    alias: 'widget.natmodelstore',
    ...
    CreateModel: function() {
        var currModel = app.CreateModel(this.currModel);
    ...
    SetModel: function (model) {
        ...
        if (this.currModel) {
            this.currModel.un('dirtyChanged', this.OnModelDirtyChanged, this);
            this.currModel.un('changed', this.OnModelChanged, this);
        }

        this.currModel = model;
        if (this.currModel) {
            this.currModel.on('dirtyChanged', this.OnModelDirtyChanged, this);
            this.currModel.on('changed', this.OnModelChanged, this);
            this.proxy.setModel(this.currModel.self);
        }
    ...
    Save: function (op, callback, scope) {
        ...
        if ((!this.currModel) || (!this.currModel.isNATDirty)) {
            this.SaveCompleted({});
            return;
        }
        ...
        var operation = Ext.create('Ext.data.Operation', {
            ...
            action: this.currModel.phantom ? 'create' : 'update',
            needsSync: true
        });

        this.proxy[operation.action](operation, this.onProxyWrite, this);
    },

    SaveCompleted: function (operation) {
        this.saving = false;
        this.fireEvent('SaveCompleted', operation);
        Ext.callback(this.callback, this.scope, [null, operation], 0);
    },
    ...
    onCreateRecords: function(records, operation, success) {
        ...
        var original = operation.records[0];
        if (original) {
    ...
    Merge: function(source) {
        this.currModel.Merge(source.currModel);
    },

    reject: function (silent) {
        if (!this.hasModel()) return;
        this.currModel.reject(silent);
    },

    commit: function (silent) {
        if (!this.hasModel()) return;
        this.currModel.commit(silent);
    },

    IsChanged: function () {
        return (this.currModel) && (this.currModel.isNATDirty);
    },
    ...
    hasModel: function() {
        return ((this.currModel) && (typeof this.currModel == 'object') && (!this.currModel.removed));
    }
});
...
//        this.filter('deleted', false);  //snapshot gets wrong!
        ...
        //this.lastParams.lastModified = new Date(0);
        ...
        this.lastParams = Ext.merge(defaultParams, config.params);
    ...
    onProxyLoad: function (operation) {
        ...
        me.fireEvent('read', me, records, successful, operation);
    ...
    ProcessResponse: function (models, operation, success) {
        if (!success) return;

        this.MergeModels(models);
    ...
    MergeModel: function (model) {
        ...
        //updated node
        original.state = Ext.ModelState.LOADING;
    ...
    //remove method would put the record in the removed array and sync to true
    ...
    //support events: change, dirtyChanged
    insert: function(index, records) {
        ...
        for (i = 0, len = records.length; i < len; i++) {
            ...
            //IZS BEGIN
            record.on('changed', this.OnModelChanged, this);
            record.on('dirtyChanged', this.OnModelDirtyChanged, this);
            //IZS END
...
//        this.filter('deleted', false);  //snapshot get wrong!
    ...
    ProcessResponse: function (models, operation, success) {
        ...
        var firstLoad = this.lastParams.lastModified.getTime() == new Date(0).getTime();

        if (firstLoad) {
            this.BuildStore(models);
        }
        else {
            this.MergeModels(models);
        }
...
//        this.removeAll();
        ...
        for (var i = 0; i < models.length; i++) {
            var model = models[i];
            model.on('changed', this.Model_changed(model), this);
        }
    ...
    Model_changed: function(model) {
        return function(fieldName, value) {
            this.fireEvent('changed', model, fieldName, value);
        }
    },
    ...
    MergeModel: function (model) {
        ...
        if (!parentId) { //root node
            ...
            root.state = Ext.ModelState.LOADING;
...
Ext.define('natjs.NATScriptRunner', {
    alias: 'widget.NATScriptRunner',

    CreateScript: function(source) {
        var head = document.getElementsByTagName('head')[0];
        var runner = document.getElementById("runner");
        if (runner) {
            head.removeChild(runner);
        }

        var script = document.createElement('script');
        script.setAttribute("type","text/javascript");
        script.setAttribute("id", "runner");
//        if (debugmode) {
//            source = "debugger; \n " + source;
//        }
//        script.innerHTML = "function Main(" + args + ") { \n " + source + " \n };";
        script.innerHTML = source;
        head.appendChild(script);
    },

    RunScript: function(fn, args) {
        return fn(args);
    }
});
